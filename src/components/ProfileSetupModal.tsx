...
const ProfileSetupModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use AuthContext's updateProfile so local state updates immediately
      await updateProfile({
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '2rem', marginBottom: '0.5rem' }}>
              Welcome to <span style={{ color: 'var(--primary)' }}>IntraSpace</span>
            </h2>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
              Just one last step — let us personalize your experience.
            </p>
          </div>
